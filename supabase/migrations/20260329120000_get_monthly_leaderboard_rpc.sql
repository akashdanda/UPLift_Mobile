-- Monthly leaderboard must run with privileges that bypass RLS. Client-side aggregation
-- used .from('friendships') and .from('group_competitions') which only return rows the
-- viewer is allowed to see, so different users/devices saw different point totals.
...
DECLARE
  ...
  v_my_row jsonb;
BEGIN
  -- Match lib/leaderboard.ts getMonthBounds() — calendar month in UTC
  ...
  winning_comps AS (
    SELECT c.winner_group_id,
           COUNT(*)::integer AS win_count
    FROM public.group_competitions c
    WHERE c.status = 'completed'::competition_status
      AND c.winner_group_id IS NOT NULL
      AND (c.ends_at AT TIME ZONE 'UTC')::date >= v_date_start
      AND (c.ends_at AT TIME ZONE 'UTC')::date <= v_date_end
    GROUP BY c.winner_group_id
  ),
  user_comp_wins AS (
    SELECT gm.user_id,
           COALESCE(SUM(wc.win_count), 0)::integer AS competition_wins
    FROM winning_comps wc
    JOIN public.group_members gm ON gm.group_id = wc.winner_group_id
    GROUP BY gm.user_id
  ),
  friend_adds AS (
    SELECT f.uid,
  ...
  all_user_ids AS (
    ...
    UNION
    SELECT user_id FROM user_comp_wins
    UNION
  ...
  scored AS (
    SELECT
      ...
      COALESCE(v.competition_wins, 0) AS competition_wins,
      COALESCE(f.friends_count, 0) AS friends_count,
      COALESCE(g.groups_count, 0) AS groups_count,
      (
        COALESCE(f.friends_count, 0) * 2
        + COALESCE(g.groups_count, 0) * 1
        + COALESCE(w.workouts_count, 0) * 5
        + COALESCE(v.competition_wins, 0) * 20
      )::integer AS points,
      p.display_name,
      p.avatar_url
    FROM all_user_ids u
    LEFT JOIN workout_counts w ON w.user_id = u.user_id
    LEFT JOIN user_comp_wins v ON v.user_id = u.user_id
